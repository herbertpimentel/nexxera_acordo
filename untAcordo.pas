...
begin
    ...
    begin
        ShowMessage('Você precisa salvar os dados do escritório');
        Exit;
    end;
...
//    showmessage(StringRightPad('beto', 2, ' ')); exit;
    ...
    // se houverem mais pendencias que parcelas são geradas linhas extras para pendencia
    ...
    begin
        ...
        // quando o numero de pendencias/contratos é maior queo numero de parcelas
        // devem ser validados apenas os campos até posição 65
        if (DM.TbPendencia.RecNo > qtd_parcelas) then
        begin
            // quebra a string apenas porque o DELPHI tem a limitação de 255 caracteres por linnha
            linha := linha
        ...
        begin
            ...
            // valor da parcela (se for a 1, então é o valor dado no ato, senao é o valor das demais parcelas)
        ...
        // passa para proxima pendência
        DM.TbPendencia.Next;
    end;

    SL.SaveToFile(caminho);

    // atualiza o contador de arquivos gerados pelo escritório
